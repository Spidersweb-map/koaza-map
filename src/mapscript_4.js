function initMap(src) {
    ...
    const getMapLabelLayout = (fontWeight) => {
        const mapLabelLayout = {
            ...
            'text-size': ['interpolate', ['exponential', 2], ['zoom'], 
                13.5, ['*', ['to-number', ['get', 'label_size'], 8], 0.2], 
                17, ['*', ['to-number', ['get', 'label_size'], 9], 2.0]
            ],
    ...
    const loadLayers = async() => {
        ...
        map.addSource('map_mesh', {type: 'geojson', data: src + '/map_mesh.geojson'});
        // レイヤー追加
        map.addLayer({
    ...
    map.on('click', 'big_name', (e) => {
        if (e.features[0].properties.region != '(旧村)') {
            new maplibregl.Popup({closeButton: false})
                .setLngLat(e.lngLat)
                .setHTML(
